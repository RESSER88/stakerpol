...
BEGIN
  ...
  -- Nastepny kontakt: +14 dni, tylko gdy nie ma zaplanowanego przyszlego terminu
  UPDATE public.contacts c
  SET termin_followup = (now() AT TIME ZONE 'Europe/Warsaw')::date + 14,
      termin_followup_note = CASE
        WHEN c.termin_followup_note IS NULL OR btrim(c.termin_followup_note) = ''
          THEN 'Kontakt po wyslanej ofercie'
        ELSE c.termin_followup_note
      END
  WHERE c.id = _contact_id
    AND (
      c.termin_followup IS NULL
      OR c.termin_followup < (now() AT TIME ZONE 'Europe/Warsaw')::date
    );
